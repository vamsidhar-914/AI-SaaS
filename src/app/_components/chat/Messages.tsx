...
export default function Messages({ fileId }: { fileId: string }){
    ...
    const { data,isLoading,fetchNextPage } = api.file.getAllMessages.useInfiniteQuery({
        fileId,
        ...
        <div className="flex max-h-[calc(100vh-3.5rem-7rem)] border-zinc-200 flex-1 flex-col-reverse gap-4 p-3 overflow-y-auto scrollbar-thumb scrollbar-thumb-rounded scrollabar-track-blue-lighter scrollbar-w-2 scrollbar-touch">
            {combinedMessages && combinedMessages.length > 0 ? (
                combinedMessages.map((message,index) => {
                    ...
                    const isNextMsgSamePerson = combinedMessages[index -1]?.isUserMessage === combinedMessages[index]?.isUserMessage

                    if(index == combinedMessages.length -1){
                        return <Message key={message.id} isNextMsgSamePerson={isNextMsgSamePerson} message={message} />
                    }else return <Message message={message} isNextMsgSamePerson={isNextMsgSamePerson}  key={message.id}/>
                })
            ) : isLoading ? (
